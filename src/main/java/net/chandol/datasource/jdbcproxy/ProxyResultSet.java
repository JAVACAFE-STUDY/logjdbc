...
public class ProxyResultSet implements ResultSet {
    ...
    private ResultSetCollector collector;
    private LoggableDataSourceConfig config;

    ProxyResultSet(LoggableDataSourceConfig config, ResultSet resultSet) {
        this.config = config;
        this._resultSet = resultSet;

        try {
            this.collector = new ResultSetCollector(getMetaData());
    ...
    public boolean next() throws SQLException {
        ...
        // 다음 값이 있는 경우 현재 값들을 collector로 넣어줍니다.
        if (hasNext)
            collector.collectCurrentCursorResultSetData(this);
